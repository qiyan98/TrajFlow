...
def create_infos_from_protos(raw_data_path, output_path, num_workers=16):
    ...
    val_inter_infos = get_infos_from_protos(
    ...
    val_inter_filename = os.path.join(output_path, 'processed_scenarios_val_inter_infos.pkl')
    with open(val_inter_filename, 'wb') as f:
        pickle.dump(val_inter_infos, f)
    print('----------------Waymo info val inter file is saved to %s----------------' % val_inter_filename)
