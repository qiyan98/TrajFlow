...
#####################################################################################
# Code is based on the BeTop model (https://arxiv.org/abs/2409.18031) implementation
# from https://github.com/OpenDriveLab/BeTop by Haochen Liu, Li Chen, Yu Qiao, Chen Lv, Hongyang Li
####################################################################################
...
from trajflow.datasets.dataset import DatasetTemplate
from trajflow.utils import common_utils
from trajflow.config import init_cfg


class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, testing=False, inter_pred=False, logger=None):
        super().__init__(dataset_cfg=dataset_cfg, training=training, testing=testing, inter_pred=inter_pred, logger=logger)
        _init_cfg = init_cfg()
        self.data_root = _init_cfg.ROOT_DIR / self.dataset_cfg.DATA_ROOT
        ...
        self.logger.info(f'Mode: {self.mode}. Total scenes after filters: {len(self.infos)}')
    ...
    def create_scene_level_data(self, index):
        ...
        if not self.dataset_cfg.get('WITHOUT_HDMAP', False):
            ...
            # debug: use dummy values to replace road map
            ret_dict['map_polylines'] = np.zeros((len(center_objects), 1, 20, 9), dtype=np.float32)
            ret_dict['map_polylines_mask'] = np.ones((len(center_objects), 1, 20), dtype=bool)
            ret_dict['map_polylines_center'] = np.zeros((len(center_objects), 1, 3), dtype=np.float32)
    ...
    def generate_centered_trajs_for_agents(self, center_objects, obj_trajs_past, obj_types, center_indices, sdc_index, timestamps, obj_trajs_future):
        ...
        ret_obj_valid_mask_future = obj_trajs_future[:, :, :, -1]  # (num_center_obejcts, num_objects, num_timestamps_future)
    ...
    def generate_prediction_dicts(self, batch_dict, inter_pred=False, flag_submission=False):
        ...
        """rotate coordinates back to world coordinates"""
        ...
        """save the pre-nms trajectory prediction in world coordinates"""
        pred_dict_list = []
        ...
        for bs_idx in range(batch_dict['batch_size']):
            ...
            for obj_idx in range(start_obj_idx, start_obj_idx + batch_sample_count[bs_idx]):
                single_pred_dict = {
                    ...
                    'pred_trajs': pred_trajs_world[obj_idx, :, :, 0:2].cpu().numpy() if not inter_pred else pred_trajs_world[obj_idx, :, :, 0:2],
                    'pred_scores': pred_scores[obj_idx, :].cpu().numpy() if not inter_pred else pred_scores[obj_idx, :],
                ...
                if 'center_gt_trajs_src' in input_dict:
                    single_pred_dict['gt_trajs'] = input_dict['center_gt_trajs_src'][obj_idx].cpu().numpy()

                if flag_submission and (not inter_pred):
                    sort_idxs = single_pred_dict['pred_scores'].argsort()[::-1]
                    single_pred_dict['pred_scores'] = single_pred_dict['pred_scores'][sort_idxs][:6]
                    single_pred_dict['pred_scores'] += np.argsort(np.argsort(single_pred_dict['pred_scores']))  # Break ties
                    single_pred_dict['pred_trajs'] = single_pred_dict['pred_trajs'][sort_idxs][:6]


                # save the trajectory in the normalized coordinates
                # single_pred_dict.update({
                #     'pred_trajs_normalized': pred_trajs[obj_idx, :, :, 0:2].cpu().numpy(),
                # })

                cur_scene_pred_list.append(single_pred_dict)
                # the end of the loop for each agent

            if inter_pred:
                cur_scene_pred_list = self.generate_joint_prediction(cur_scene_pred_list, flag_submission)
    ...
    def evaluation(self, pred_dicts, eval_method='waymo', eval_no_nms=False, inter_pred=False, **kwargs):
        if eval_method == 'waymo':
            from .waymo_eval import waymo_evaluation
            if eval_no_nms:
                num_modes_for_eval = pred_dicts[0][0]['pred_trajs'].shape[0]  # e.g., 64
            else:
